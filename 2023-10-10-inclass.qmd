...
```{r}
ggplot(gender, aes(x = rank, y = percentage, fill = sex)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Rank", y = "Percentage (%)", fill = "Sex") +
  theme_minimal() +
  theme(legend.position = "top")
```

```{r}
ggplot(d, aes(x = age, y = score, color = sex)) +
  geom_point() +
  geom_smooth()+
  labs(x = "Age", y = "Score") +
  theme_minimal()
...
```{r}
reg <- lm(score ~ sex*age, data = d)
broom::tidy(reg)
```

```{r}
model <- lm(score ~ age, data = d)
...
```{r}
male_data <- d %>% filter(sex == "male")
female_data <- d %>% filter(sex == "female")
```

```{r}
male_model <- lm(score ~ age, data = male_data)
broom :: tidy(male_model)
```

```{r}
female_model <- lm(score ~ age, data = female_data)
broom :: tidy(female_model)
```
